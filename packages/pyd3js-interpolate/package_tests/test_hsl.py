...
from pyd3js_interpolate import interpolateHsl


def test_interpolate_hsl() -> None:
    assert interpolateHsl("steelblue", "brown")(0) == str(rgb("steelblue"))
    assert interpolateHsl("steelblue", hsl("brown"))(1) == str(rgb("brown"))
    assert interpolateHsl("steelblue", rgb("brown"))(1) == str(rgb("brown"))

    assert interpolateHsl("steelblue", "#f00")(0.2) == "rgb(56, 61, 195)"
    assert (
        interpolateHsl("rgba(70, 130, 180, 1)", "rgba(255, 0, 0, 0.2)")(0.2)
        == "rgba(56, 61, 195, 0.84)"
    )
