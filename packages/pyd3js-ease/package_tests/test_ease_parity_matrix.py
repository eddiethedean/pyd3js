...
def _read_readme_matrix() -> dict[str, str]:
    ...
    in_section = False
    for line in md.splitlines():
        if line.strip() == "### Upstream exports (`d3-ease@3.0.1`)":
            in_section = True
            continue
...
def test_readme_matrix_matches_python_exports() -> None:
    ...
    assert missing_from_python == [], (
        "README marks implemented but not exported by pyd3js_ease: "
    ...
    assert unexpected_exports == [], (
        f"Unexpected pyd3js_ease exports: {unexpected_exports}"
    )
