...
def test_readme_matrix_matches_python_exports() -> None:
    matrix = _read_readme_matrix()
    exported = set(pyd3js_dispatch.__all__)
    ...
    assert missing_from_python == [], (
        "README marks implemented but not exported by pyd3js_dispatch: "
    ...
    assert unexpected_exports == [], (
        f"Unexpected pyd3js_dispatch exports: {unexpected_exports}"
    )
