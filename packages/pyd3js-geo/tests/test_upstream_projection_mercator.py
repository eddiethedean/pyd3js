...
def test_mercator_clip_extent_intersects_specified_extent():
    projection = geoMercator().translate([0, 0]).scale(1).clipExtent([[-10, -10], [10, 10]]).precision(0)
    assert_path_equal(
        geoPath(projection)({"type": "Sphere"}),
...
def test_mercator_rotate_does_not_affect_automatic_clip_extent():
    ...
    assert_in_delta(projection.scale(), 20969742.365692537, 1e-6)
    assert_in_delta(projection.translate(), [30139734.76760269, 11371473.949706702], 1e-6)
    projection.rotate([0, 95]).fitExtent([[0, 0], [960, 600]], obj)
    assert_in_delta(projection.scale(), 35781690.650920525, 1e-6)
    assert_in_delta(projection.translate(), [75115911.95344563, 2586046.4116968135], 1e-6)


def test_transverse_mercator_clip_extent_null_sets_default_automatic_clip_extent():
    projection = geoTransverseMercator().translate([0, 0]).scale(1).clipExtent(None).precision(0)
    assert_path_equal(
        geoPath(projection)({"type": "Sphere"}),
...
def test_transverse_mercator_center_sets_automatic_clip_extent():
    projection = geoTransverseMercator().translate([0, 0]).scale(1).center([10, 10]).precision(0)
    assert_path_equal(
        geoPath(projection)({"type": "Sphere"}),
...
def test_transverse_mercator_clip_extent_intersects_specified_extent():
    projection = geoTransverseMercator().translate([0, 0]).scale(1).clipExtent([[-10, -10], [10, 10]]).precision(0)
    assert_path_equal(
        geoPath(projection)({"type": "Sphere"}),
