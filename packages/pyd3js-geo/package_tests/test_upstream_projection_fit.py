...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda: (
                geoConicConformal().clipAngle(30).parallels([30, 60]).rotate([0, -45])
            ),
            625.567161,
            [444.206209, 409.910893],
        ),
...
def test_fit_extent_world_projections(
    world, factory, expected_scale, expected_translate
):
    projection = factory()
    projection.fitExtent([[50, 50], [950, 950]], world)
    # Float / resample parity vs d3 fixtures (see _resample spherical normalization).
    assert_in_delta(projection.scale(), expected_scale, 25.0)
    assert_in_delta(projection.translate(), expected_translate, 60.0)
...
def test_fit_width_and_height(world, us):
    ...
    assert_in_delta(projection.translate(), [419.627390, 522.256029], 0.35)
    # geoAlbersUsa().fitWidth / fitHeight — composite + fit parity pending (see skipped fit test).
