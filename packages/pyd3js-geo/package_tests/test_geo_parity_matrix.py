...
def _read_upstream_exports() -> list[str]:
    ...
    assert names, "No upstream exports found in docs/UPSTREAM_API.md ## Exports"
    return names
...
def test_readme_matrix_matches_python_exports() -> None:
    matrix = _read_readme_matrix()
    exported = set(pyd3js_geo.__all__)
    ...
    assert missing_from_python == [], (
        "README marks implemented but not exported by pyd3js_geo: "
    ...
    assert unexpected_exports == [], (
        f"Unexpected pyd3js_geo exports: {unexpected_exports}"
    )
